...
class nh_calculator():
    ...
    def car_idx(self, x):
        ...
        models = [item for item in models if item != [None, None]]
    ...
    def fetch_calculator_parameters(self, input_data, single=False):
        ...
        if single == True:
            ...
            self.sheet.range('AY25').value = input_data['prepayment_price'] # 선수금 비율
            self.sheet.range('AY27').value = input_data['deposit_price'] # 보증금 비율
