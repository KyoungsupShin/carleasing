...
class dgb_calculator():
    ...
    def initialize_data(self):
        ...
        self.sheet.range('AS43').value = 0 # CM인센티브 비율
    ...
    def fetch_calculator_parameters(self, input_data, single=False):
        ...
        self.sheet.range('BD24').value = input_data['etc_price'] #할인가격
        self.sheet.range('BR21').value = input_data['etc_yn'] #할인가격
        self.sheet.range('AS43').value = 0 # CM인센티브 비율

        if single == True:
            self.sheet.range('AS28').value = input_data['lease_month'] #리스기간 (반복 실행)
            self.sheet.range('AN40').value = input_data['distance'] #운행거리 (반복 실행)
            self.sheet.range('BD33').value = input_data['prepayment_price'] # 선수금 비율
            self.sheet.range('BD30').value = input_data['deposit_price'] # 보증금 비율
        ...
        if single == True:
            if input_data['max_res_yn'] == True:
                ...
                limit_sum = limit_sum if limit_sum < self.sheet.range('AS38').value + 0.01 else self.sheet.range('AS38').value  
                self.sheet.range('AS36').value = limit_sum 
            else:
                if input_data['residual_rate'] < 0.3:
                    self.sheet.range('AS36').value = 0.3
                else:
                    if input_data['residual_rate'] <= self.sheet.range('AS38').value:
                        self.sheet.range('AS36').value = input_data['residual_rate'] #잔가 (세부 선택값)
                    else:
                        limit_sum = 1 - (self.sheet.range('AS30').value + self.sheet.range('AS33').value)
                        limit_sum = limit_sum if limit_sum < self.sheet.range('AS38').value + 0.01 else self.sheet.range('AS38').value  
                        self.sheet.range('AS36').value = limit_sum 
    ...
    def create_single_report(self):
        report = {
                    ...
                    "최대잔가" : round(self.sheet.range('AS36').value*100,2),
